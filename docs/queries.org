...
A result is the node =org-node-read= returns for the same heading, without its
=content= and its =children= — a match list would read every matched subtree to
fill those — and with the heading's Org property drawer as =properties=. Both
tag fields come with it, so a client about to replace a heading's tags works
from =local_tags= without reading the heading again (see [[file:reading.org::*The two tag fields][The two tag fields]]).

** query-inbox
- *Description:* Query inbox items using the configured GTD workflow
- *Parameters:* None
- *Returns:* Same as =org-query=, sorted by =org-mcp-query-sort-fn= when configured
