# A done keyword leaves an org-mcp clock open
...
reaching a done keyword leaves the line open and reports no `clock`. org-mcp
...
whatever else that function comes to consult, read and applied by org-mcp rather
...
out of step. org-mcp reports what Org decided; it does not decide in Org's place.

We chose it over reaching for `org-mcp--clock-resolve-dangling`, which deletes an
...
The cost is that a client clocking a task through org-mcp and then finishing it
