...
A named file must also meet these conditions:

- it is an existing =.org= or =.org_archive= file; encrypted =.org.gpg= files
  and files with other extensions are refused;
- it is local: remote (TRAMP) paths are refused before Emacs touches them, so a
  path cannot make Emacs open a network connection;
