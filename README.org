...
*Note:* The =org-id://= resource template has been merged into =org-headline://=. Use =org-headline://{uuid}= to access headlines by ID. Tools still accept =org-id://= URIs for backward compatibility.
...
- *Returns:* Success status with arrays of =properties_set= and =properties_deleted=, plus ID-based URI
...
- *Returns:* Success status with =previous_tags= and =new_tags= arrays, plus ID-based URI
...
- *Returns:* Success status with ID-based URI
