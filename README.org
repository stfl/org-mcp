...
(use-package org-mcp
  :after org
  :custom
  ;; Entries may be absolute or relative paths.  Relative entries are
  ;; resolved against `org-directory' (same convention as
  ;; `org-agenda-files'), so the two lists can be reused directly.
  (org-mcp-allowed-files org-agenda-files)
...
Once you read and internalized the warning above, set the allowed Org file set.
Entries may be absolute or relative paths.  Relative entries are resolved against
=org-directory= (matching =org-agenda-files= behavior), and tilde / environment
variable expansion applies.  Absolute and relative entries can be mixed freely:

#+begin_src emacs-lisp
;; Absolute paths
(setq org-mcp-allowed-files '("/path/to/foo.org" "/path/to/bar.org"))

;; Relative paths (resolved against `org-directory')
(setq org-directory "~/org")
(setq org-mcp-allowed-files '("inbox.org" "projects/work.org"))

;; Reuse `org-agenda-files' directly
